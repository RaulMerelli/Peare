...
namespace Peare
{
    public static class LxResources
    {
        ...
        public static byte[] OpenResourceLX(string typeName, string targetResourceName, out string message, out bool found)
        {
            ...
            IMAGE_LX_HEADER header = Program.Deserialize<IMAGE_LX_HEADER>(headerBytes);
            ...
            for (int i = 0; i < header.NumResourceTableEntries; i++)
            {
                ...
                if (currentTypeName == typeName && nameID.ToString() == targetResourceName)
                {
                    ...
                    LX_OBJECT_TABLE_ENTRY tableEntry = Program.Deserialize<LX_OBJECT_TABLE_ENTRY>(tableEntryBytes);
                    ...
                    while (bytesRead < resourceSize && currentPageIndexInObject < tableEntry.PageTableEntries)
                    {
                        ...
                        if (pageFlags == 0x01) // Iterated Data Page
                        {
                            ...
                            try
                            {
                                Program.DumpRaw(compressedBytes);
                                ...
                                Program.DumpRaw(decompressedPageData);
