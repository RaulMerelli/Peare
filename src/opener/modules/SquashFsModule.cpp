...
ModulePtr SquashFsModule::open(const fs::ByteStorePtr& disc, const QString& sourceName,
                               const QString& subPath) {
    auto module = peare::makeUnique<SquashFsModule>();
    module->info_.filePath = sourceName;
    module->info_.format = ModuleFormat::SQUASHFS;

    fs::SquashFsReader reader(disc);
    if (!reader.valid()) {
    ...
    buildFsLevel(reader, disc, subPath.toStdString(), ModuleFormat::SQUASHFS,
                 QStringLiteral("SQUASHFS_FILE"), QStringLiteral("SQUASHFS_DIR"),
                 module->resources_);
    return ModulePtr(std::move(module));
}

ModulePtr SquashFsModule::open(const QString& filePath) {
