...
ModulePtr VhdModule::buildFromDisk(const fs::ByteStorePtr& disk, const QString& sourceName,
                                   const QString& error) {
    auto module = peare::makeUnique<VhdModule>();
    module->info_.filePath = sourceName;
    module->info_.format = ModuleFormat::VHD;
    module->info_.description = QStringLiteral("Microsoft Virtual Hard Disk");
    if (!disk) {
        module->info_.error = error.isEmpty() ? QStringLiteral("Cannot open VHD") : error;
...
ModulePtr VhdModule::open(const fs::ByteStorePtr& file, const QString& sourceName) {
    std::string err;
    fs::ByteStorePtr disk = fs::openVhdDisk(file, &err);
    return buildFromDisk(disk, sourceName, QString::fromStdString(err));
}

ModulePtr VhdModule::open(const QString& filePath) {
