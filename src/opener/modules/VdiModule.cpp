...
ModulePtr VdiModule::buildFromDisk(const fs::ByteStorePtr& disk, const QString& sourceName,
                                   const QString& error) {
    auto module = peare::makeUnique<VdiModule>();
    module->info_.filePath = sourceName;
    module->info_.format = ModuleFormat::VDI;
    module->info_.description = QStringLiteral("VirtualBox Disk Image");
    if (!disk) {
        module->info_.error = error.isEmpty() ? QStringLiteral("Cannot open VDI") : error;
...
ModulePtr VdiModule::open(const fs::ByteStorePtr& file, const QString& sourceName) {
    std::string err;
    fs::ByteStorePtr disk = fs::openVdiDisk(file, &err);
    return buildFromDisk(disk, sourceName, QString::fromStdString(err));
}

ModulePtr VdiModule::open(const QString& filePath) {
