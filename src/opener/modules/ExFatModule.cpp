...
ModulePtr ExFatModule::open(const fs::ByteStorePtr& disc, const QString& sourceName,
                            const QString& subPath) {
    auto module = peare::makeUnique<ExFatModule>();
    module->info_.filePath = sourceName;
    module->info_.format = ModuleFormat::EXFAT;
    module->info_.description = QStringLiteral("exFAT volume");

    auto reader = std::make_shared<fs::ExFatReader>(disc);
    if (!reader->valid()) {
    ...
    buildFsLevel(*reader, disc, subPath.toStdString(), ModuleFormat::EXFAT,
                 QStringLiteral("EXFAT_FILE"), QStringLiteral("EXFAT_DIR"), module->resources_);
    return ModulePtr(std::move(module));
}

ModulePtr ExFatModule::open(const QString& filePath) {
