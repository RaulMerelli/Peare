...
ModulePtr JfsModule::open(const fs::ByteStorePtr& disc, const QString& sourceName,
                          const QString& subPath) {
    auto module = peare::makeUnique<JfsModule>();
    module->info_.filePath = sourceName;
    module->info_.format = ModuleFormat::JFS;

    fs::JfsReader reader(disc);
    if (!reader.valid()) {
    ...
    buildFsLevel(reader, disc, subPath.toStdString(), ModuleFormat::JFS,
                 QStringLiteral("JFS_FILE"), QStringLiteral("JFS_DIR"), module->resources_);
    return ModulePtr(std::move(module));
}

ModulePtr JfsModule::open(const QString& filePath) {
