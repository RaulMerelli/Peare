...
ModulePtr XfsModule::open(const fs::ByteStorePtr& disc, const QString& sourceName,
                          const QString& subPath) {
    auto module = peare::makeUnique<XfsModule>();
    module->info_.filePath = sourceName;
    module->info_.format = ModuleFormat::XFS;

    fs::XfsReader reader(disc);
    if (!reader.valid()) {
    ...
    buildFsLevel(reader, disc, subPath.toStdString(), ModuleFormat::XFS,
                 QStringLiteral("XFS_FILE"), QStringLiteral("XFS_DIR"), module->resources_);
    return ModulePtr(std::move(module));
}

ModulePtr XfsModule::open(const QString& filePath) {
