...
bool hasVisibleLanguage(const QString& language)
{
    ...
    if (value.compare(QStringLiteral("neutral"), Qt::CaseInsensitive) == 0)
        return false;
...
void MainWindow::buildMenus()
{
    ...
    connect(issueAction, &QAction::triggered, this, [] {
        QDesktopServices::openUrl(QUrl(QStringLiteral("https://github.com/RaulMerelli/Peare/issues")));
    });
...
void MainWindow::buildCentralUi()
{
    ...
    connect(treeView_, &QTreeWidget::itemSelectionChanged,
    ...
    connect(treeView_, &QTreeWidget::itemCollapsed, this,
            [this](QTreeWidgetItem* item) {
                if (item && item->childCount() > 0)
...
void MainWindow::showModuleSummary(const QString& filePath)
{
    ...
    std::function<void(QTreeWidgetItem*)> normalizeExpandableIcons =
        [&](QTreeWidgetItem* item) {
            if (!item) return;
            if (item->childCount() > 0)
                item->setIcon(0, settingsIcons_.folderCloseIcon());
            for (int i = 0; i < item->childCount(); ++i)
                normalizeExpandableIcons(item->child(i));
        };
    ...
    exportOriginalAction_->setEnabled(false); exportConvertedAction_->setEnabled(false);
...
static QImage cropVisibleGlyph(const QImage& source)
{
    ...
    for (int y=0; y<source.height(); ++y) for (int x=0; x<source.width(); ++x) {
        ...
        left=qMin(left,x); top=qMin(top,y); right=qMax(right,x); bottom=qMax(bottom,y);
    }
    return right >= left && bottom >= top ? source.copy(left,top,right-left+1,bottom-top+1) : source;
}

QImage MainWindow::buildFontGlyphMap(const pearegui::Resource& resource, const QString& resourceType) const
{
    quint64 first = 0, last = 0;
    if (!pearegui::getUnsigned(resource, PEARE_INFO_FONT_FIRST_CHARACTER, &first, resourceType) ||
        !pearegui::getUnsigned(resource, PEARE_INFO_FONT_LAST_CHARACTER, &last, resourceType)) return {};
    if (last < first || last - first + 1 > 65536) return {};
    const int scale = 1;
    QVector<QImage> glyphs; glyphs.reserve(int(last-first+1));
    ...
    for (quint64 code=first; code<=last; ++code) {
        QImage glyph=pearegui::renderFont(resource,QString(QChar(static_cast<ushort>(code))),scale,0,
                                          0x000000FFu,0xFFFFFF00u,resourceType);
    ...
    QImage result(columns*cellWidth+1,rows*cellHeight+1,QImage::Format_ARGB32); result.fill(Qt::white);
    QPainter painter(&result); QFont codeFont(QStringLiteral("Monospace"),7); codeFont.setStyleHint(QFont::TypeWriter);
    for (int i=0;i<glyphs.size();++i) {
        const int x=(i%columns)*cellWidth,y=(i/columns)*cellHeight; painter.setPen(QColor(Qt::lightGray)); painter.drawRect(x,y,cellWidth,cellHeight);
        QImage glyph=glyphs.at(i); if(!glyph.isNull()){if(glyph.width()>imageAreaWidth||glyph.height()>imageAreaHeight)glyph=glyph.scaled(imageAreaWidth,imageAreaHeight,Qt::KeepAspectRatio,Qt::FastTransformation);painter.drawImage(x+6+qMax(0,(imageAreaWidth-glyph.width())/2),y+2+qMax(0,(imageAreaHeight-glyph.height())/2),glyph);}
        painter.setFont(codeFont); painter.setPen(QColor(Qt::darkGray)); const quint64 code=first+quint64(i);
        painter.drawText(x+2,y+imageAreaHeight+3,cellWidth-4,14,Qt::AlignLeft|Qt::AlignVCenter,QString::number(code,16).toUpper().rightJustified(code<=0xFF?2:4,QLatin1Char('0')));
    }
    return result;
}
...
void MainWindow::showSelectedResource()
{
    ...
    clearContent(); messageLabel_->clear(); if(!item->data(0,TypeRole).isValid())return;
    ...
    bool isFontResource = valid && pearegui::getUnsigned(
        resource, PEARE_INFO_FONT_FIRST_CHARACTER, &fontFirst, effectiveResourceType);
    if (!isFontResource && valid && !effectiveResourceType.isEmpty()) {
        // Retry without a declared type so get_info() can classify raw payloads.
        isFontResource = pearegui::getUnsigned(
            resource, PEARE_INFO_FONT_FIRST_CHARACTER, &fontFirst, {});
    }
    if (isFontResource) {
    ...
    if(isFontResource) {
        ...
        for (quint32 scale=1; scale<=4; ++scale) {
            const QImage image=pearegui::renderFont(resource,sample,scale,8,0x000000FFu,0xFFFFFFFFu,effectiveResourceType);
            if(!image.isNull()) preview.images.push_back(image);
        }
        const QImage map=buildFontGlyphMap(resource,effectiveResourceType); if(!map.isNull()) preview.images.push_back(map);
    }
    if (valid && type == QStringLiteral("PE_MODULE")) {
        const auto context = resource.context();
        preview.error.clear(); preview.rawDump=false; preview.images.clear(); preview.pngs.clear(); preview.texts.clear();
        preview.text = QStringLiteral("Loaded PE image\nName: %1\nSize: %2 bytes\n\nExport original preserves the memory layout.\nExport converted rebuilds a standard PE file layout.").arg(context.identifier).arg(resource.payload().size());
    } else if(valid && (!preview.error.isEmpty() || (preview.images.isEmpty()&&preview.text.isEmpty()))) {
        preview.images.clear(); preview.pngs.clear(); preview.texts.clear();
        if (item->childCount() > 0) {
    ...
    for(int i=0;i<preview.images.size();++i){
        if (isFontPreview && i < 4) {
            ...
            QFont labelFont = scaleLabel->font(); labelFont.setBold(true); scaleLabel->setFont(labelFont);
            ...
            QFont labelFont = mapLabel->font(); labelFont.setBold(true); mapLabel->setFont(labelFont);
            layout->addWidget(mapLabel, 0, Qt::AlignLeft);
        }
        auto* picture=new QLabel(contentWidget_);picture->setAlignment(Qt::AlignTop|Qt::AlignLeft);picture->setPixmap(QPixmap::fromImage(preview.images[i]));picture->setFixedSize(preview.images[i].size());picture->setSizePolicy(QSizePolicy::Fixed,QSizePolicy::Fixed);layout->addWidget(picture,0,Qt::AlignTop|Qt::AlignLeft);
    }
    if(!preview.text.isEmpty()){
        auto* text=new QPlainTextEdit(contentWidget_);text->setReadOnly(true);text->setLineWrapMode(QPlainTextEdit::NoWrap);text->setFrameShape(QFrame::NoFrame);if(preview.rawDump)text->setFont(QFontDatabase::systemFont(QFontDatabase::FixedFont));text->setPlainText(preview.text);
        if(preview.images.isEmpty()){contentArea_->setHorizontalScrollBarPolicy(Qt::ScrollBarAlwaysOff);contentArea_->setVerticalScrollBarPolicy(Qt::ScrollBarAlwaysOff);text->setSizePolicy(QSizePolicy::Expanding,QSizePolicy::Expanding);layout->addWidget(text,1);}else{text->setHorizontalScrollBarPolicy(Qt::ScrollBarAlwaysOff);text->setVerticalScrollBarPolicy(Qt::ScrollBarAlwaysOff);text->setFixedHeight(qMax(1,preview.text.count(QLatin1Char('\n'))+1)*text->fontMetrics().lineSpacing()+16);layout->addWidget(text);}
    }
    if(preview.images.isEmpty()&&preview.text.isEmpty()){
        ...
        QString details = QStringLiteral("<b>%1 / %2</b><br><br>").arg(displayedType.toHtmlEscaped(), name.toHtmlEscaped());
    ...
    if(!preview.images.isEmpty()){contentWidget_->adjustSize();contentArea_->setWidget(contentWidget_);}
    if(valid){const QString ext=pearegui::originalExtension(type,resource.payload());exportOriginalAction_->setText(QStringLiteral("Export original (%1)").arg(ext));exportOriginalAction_->setEnabled(true);}
    exportConvertedMenu_->clear(); directConvertedExtension_.clear();
    ...
    if (extensions.size() == 1) {
        ...
        exportConvertedAction_->setText(QStringLiteral("Export converted (%1)").arg(directConvertedExtension_));
    } else {
        exportConvertedAction_->setText(QStringLiteral("Export converted"));
        for(const QString& ext:extensions){QAction* action=exportConvertedMenu_->addAction(QStringLiteral("%1 (%2)").arg(ext.mid(1).toUpper(),ext));connect(action,&QAction::triggered,this,[this,ext]{exportSelectedConverted(ext);});}
        exportConvertedAction_->setMenu(extensions.isEmpty() ? nullptr : exportConvertedMenu_);
    }
    messageLabel_->setText(preview.error.isEmpty()?displayedType+QStringLiteral(" / ")+name:preview.error);
}
...
void MainWindow::exportSelectedOriginal()
{
    ...
    QFile file(path);if(!file.open(QIODevice::WriteOnly)||file.write(payload)!=payload.size()){QMessageBox::warning(this,QStringLiteral("Export resource"),QStringLiteral("Cannot write resource to %1").arg(path));return;}messageLabel_->setText(QStringLiteral("Exported original resource: %1").arg(path));
}
...
void MainWindow::exportSelectedConverted(const QString& extension)
{
    ...
    if(files.size()==1){const QString path=QFileDialog::getSaveFileName(this,QStringLiteral("Export converted resource"),QDir::home().filePath(base+extension),QStringLiteral("%1 file (*%2);;All files (*.*)").arg(extension.mid(1).toUpper(),extension));if(path.isEmpty())return;QFile f(path);if(!f.open(QIODevice::WriteOnly)||f.write(files.first())!=files.first().size()){QMessageBox::warning(this,QStringLiteral("Export resource"),QStringLiteral("Cannot write converted resource to %1").arg(path));return;}messageLabel_->setText(QStringLiteral("Exported converted resource: %1").arg(path));return;}
    const QString destination=QFileDialog::getExistingDirectory(this,QStringLiteral("Select the folder for the converted resource files"),QDir::homePath());if(destination.isEmpty())return;int exported=0;for(int i=0;i<files.size();++i){const QString path=pearegui::uniquePath(QDir(destination).filePath(QStringLiteral("%1_%2%3").arg(base).arg(i+1).arg(extension)));QFile f(path);if(f.open(QIODevice::WriteOnly)&&f.write(files[i])==files[i].size())++exported;}messageLabel_->setText(QStringLiteral("Exported %1 converted resource files to %2").arg(exported).arg(destination));
}

void MainWindow::exportAllResources()
{
    if(!currentSession_.isOpen()||currentFilePath_.isEmpty())return;const QString destination=QFileDialog::getExistingDirectory(this,QStringLiteral("Select the destination folder for all original resources"),QDir::homePath());if(destination.isEmpty())return;
    const QString root=QDir(destination).filePath(pearegui::sanitizeFileName(QFileInfo(currentFilePath_).completeBaseName())+QStringLiteral("_resources"));QDir().mkpath(root);int exported=0,failed=0;
    for(size_t folder=0;folder<currentSession_.folderCount();++folder){const QString folderName=pearegui::sanitizeFileName(currentSession_.folderType(folder));const QString target=QDir(root).filePath(folderName);QDir().mkpath(target);for(size_t index=0;index<currentSession_.resourceCount(folder);++index){pearegui::Resource resource=currentSession_.openResource(folder,index);if(!resource.isValid()){++failed;continue;}const auto context=resource.context();const QByteArray payload=resource.payload();const QString path=pearegui::uniquePath(QDir(target).filePath(pearegui::sanitizeFileName(context.identifier)+pearegui::originalExtension(context.type,payload)));QFile f(path);if(!f.open(QIODevice::WriteOnly)||f.write(payload)!=payload.size()){++failed;continue;}++exported;}}
    QString summary=QStringLiteral("Exported %1 original resources to %2").arg(exported).arg(root);if(failed)summary+=QStringLiteral(". Failed: %1").arg(failed);messageLabel_->setText(summary);QMessageBox::information(this,QStringLiteral("Export all resources"),summary);
}
...
void MainWindow::showAbout()
{
    ...
    auto* projectLink = new QLabel(
        QStringLiteral("<a href=\"https://github.com/RaulMerelli/Peare\">github.com/RaulMerelli/Peare</a>"),
        &dialog);
