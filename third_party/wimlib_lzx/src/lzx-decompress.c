		      ...
		      struct lzx_lru_queue *queue)
{
	...
	/* WIM uses a one-bit shorthand for the common 32768-byte block size.
	 * Classic Microsoft LZX streams (CAB and XEX) store a fixed 24-bit
	 * uncompressed block size immediately after the 3-bit block type. */
	if (classic_stream) {
		block_size = bitstream_read_bits(istream, 8) << 16;
		block_size |= bitstream_read_bits(istream, 8) << 8;
		block_size |= bitstream_read_bits(istream, 8);
	} else if (bitstream_pop_bits(istream, 1)) {
	...
	case LZX_BLOCKTYPE_VERBATIM:
		...
		tables->lentree_empty = true;
		for (unsigned i = 0; i < LZX_LENCODE_NUM_SYMBOLS; i++) {
			if (tables->lentree_lens[i] != 0) {
				tables->lentree_empty = false;
				break;
			}
		}
		if (!tables->lentree_empty) {
		 ...
		 struct input_bitstream *istream)
{
	...
	if (length_header == LZX_NUM_PRIMARY_LENS) {
		if (unlikely(tables->lentree_empty))
			return -1;
...
static inline void
lzx_align_frame_bitstream(struct input_bitstream *istream)
{
	if (istream->bitsleft > 0)
		bitstream_ensure_bits(istream, 16);
	if (istream->bitsleft & 15)
		     ...
		     struct input_bitstream *istream)
{
	...
	unsigned window_pos = *window_pos_io;
	unsigned end;
	int match_len;
	...
	while (window_pos < end) {
		...
		if (window_pos < output_size && (window_pos & (LZX_DEFAULT_BLOCK_SIZE - 1)) == 0)
			lzx_align_frame_bitstream(istream);
	}
	*window_pos_io = window_pos;
	return 0;
}

static int
lzx_decompress_internal(const void *compressed_data, size_t compressed_size,
	       void *uncompressed_data, size_t uncompressed_size,
	       void *_ctx, bool allow_multiple_windows, bool classic_stream)
{
	...
	if (!allow_multiple_windows && uncompressed_size > ctx->max_window_size) {
	...
	u32 classic_e8_filesize = 0;
	e8_preprocessing_done = false;
	if (classic_stream) {
		/* Classic LZX has an explicit Intel E8 flag.  Unlike WIM LZX,
		 * the presence of literal 0xE8 bytes or an uncompressed block must
		 * not enable the transform by itself. */
		bitstream_ensure_bits(&istream, 1);
		if (bitstream_pop_bits(&istream, 1)) {
			u32 high = bitstream_read_bits(&istream, 16);
			u32 low = bitstream_read_bits(&istream, 16);
			classic_e8_filesize = (high << 16) | low;
			e8_preprocessing_done = classic_e8_filesize != 0;
		}
	}
	...
	while (window_pos < uncompressed_size)
	{
		...
		case LZX_BLOCKTYPE_ALIGNED:
			...
			if (!classic_stream && ctx->tables.maintree_lens[0xe8] != 0)
	...
	if (e8_preprocessing_done) {
		if (classic_stream)
			lzx_undo_e8_preprocessing_classic(uncompressed_data,
						      uncompressed_size,
						      classic_e8_filesize);
		else
...
{
	return lzx_decompress_internal(compressed_data, compressed_size,
				       uncompressed_data, uncompressed_size,
				       _ctx, false, false);
}

int
peare_lzx_decompress_contiguous(const void *compressed_data,
