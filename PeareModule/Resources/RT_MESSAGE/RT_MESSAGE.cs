            ...
            try
            {
                ...
                while (offset + 1 < data.Length) // This loop condition might need adjustment based on full resource parsing
                {
                    ...
                    if (properties.headerType == ModuleResources.HeaderType.PE)
                    {
                        ...
                        Marshal.SizeOf(typeof(uint)); // Advance offset past NumberOfBlocks

                        // The blocks array starts immediately after NumberOfBlocks
                        IntPtr blockPtr = IntPtr.Add(currentUnmanagedPtr, offset);
                        int blockSize = Marshal.SizeOf(typeof(MESSAGE_RESOURCE_BLOCK));

                        for (int i = 0; i < numberOfBlocks; i++)
                        {
                            // Read the current MESSAGE_RESOURCE_BLOCK
                            MESSAGE_RESOURCE_BLOCK block = (MESSAGE_RESOURCE_BLOCK)Marshal.PtrToStructure(blockPtr, typeof(MESSAGE_RESOURCE_BLOCK));
