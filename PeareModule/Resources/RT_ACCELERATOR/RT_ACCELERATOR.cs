...
namespace PeareModule
{
    public class RT_ACCELERATOR
    {
        ...
        private static readonly Dictionary<ushort, string> VirtualKeyCodeMap = new Dictionary<ushort, string>
        {
            { 0x01, "VK_LBUTTON" },    { 0x02, "VK_RBUTTON" },    { 0x03, "VK_CANCEL" },
            { 0x04, "VK_MBUTTON" },    { 0x05, "VK_XBUTTON1" },   { 0x06, "VK_XBUTTON2" },
            { 0x08, "VK_BACK" },       { 0x09, "VK_TAB" },        { 0x0C, "VK_CLEAR" },
            { 0x0D, "VK_RETURN" },     { 0x10, "VK_SHIFT" },      { 0x11, "VK_CONTROL" },
            { 0x12, "VK_MENU" },       { 0x13, "VK_PAUSE" },      { 0x14, "VK_CAPITAL" },
            { 0x1B, "VK_ESCAPE" },     { 0x20, "VK_SPACE" },      { 0x21, "VK_PRIOR" }, // Page Up
            { 0x22, "VK_NEXT" },       { 0x23, "VK_END" },        { 0x24, "VK_HOME" },
            { 0x25, "VK_LEFT" },       { 0x26, "VK_UP" },         { 0x27, "VK_RIGHT" },
            { 0x28, "VK_DOWN" },       { 0x2C, "VK_SNAPSHOT" },   // Print Screen
            ...
            using (BinaryReader reader = new BinaryReader(ms))
            {
                try
                {
                    ...
                    while (!isLastEntry && reader.BaseStream.Position < reader.BaseStream.Length)
                    {
                        ...
                        ushort wID = reader.ReadUInt16();             // Command ID
                        ushort wAnsiOrVirtualKey = reader.ReadUInt16(); // Character code or virtual key code
                        ...
                        {
                            // It's a virtual key accelerator.
                            rcFlags.Add("VIRTKEY"); // Add VIRTKEY flag to the RC output.

                            // Try to get a human-readable name from the map.
                            if (VirtualKeyCodeMap.TryGetValue(wAnsiOrVirtualKey, out string vkName))
                            {
                                keyString = vkName;
                            }
                            else
                            {
                                ...
                                if (wAnsiOrVirtualKey >= 0x30 && wAnsiOrVirtualKey <= 0x39) // '0'-'9'
                                {
                                    keyString = $"VK_{(char)wAnsiOrVirtualKey}";
                                }
                                else if (wAnsiOrVirtualKey >= 0x41 && wAnsiOrVirtualKey <= 0x5A) // 'A'-'Z'
                                {
                                    keyString = $"VK_{(char)wAnsiOrVirtualKey}";
                                }
                                else
                                {
                                    // Fallback for unknown or less common virtual keys: use hex representation.
                                    keyString = $"0x{wAnsiOrVirtualKey:X4}";
                                }
                            }
                        }
                        else
                        {
                            // It's a character accelerator.
                            // RC format uses double quotes for character literals (e.g., "A").
                            // Handle special characters like tab if needed, otherwise output raw character.
                            if (wAnsiOrVirtualKey == '\t')
                            {
                                keyString = "\"\\t\""; // Represent tab as "\t" in RC
                            }
                            else if (wAnsiOrVirtualKey == '\0')
                            {
                                keyString = "\"\\0\""; // Represent null as "\0" in RC
                            }
                            else if (char.IsControl((char)wAnsiOrVirtualKey) || char.IsWhiteSpace((char)wAnsiOrVirtualKey) && wAnsiOrVirtualKey != ' ')
                            {
                                // For other non-printable control characters or non-space whitespace,
                                // represent as hex to avoid issues.
                                keyString = $"0x{wAnsiOrVirtualKey:X4}";
                                rcFlags.Add("ASCII"); // Indicate it's an ASCII value, often used with non-printable chars
                            }
                            else
                            {
                                keyString = $"\"{(char)wAnsiOrVirtualKey}\"";
                            }
                        }
                        ...
                        rcContent.AppendLine($"    {keyString}, {wID}{flagsString}");
                    }
                }
